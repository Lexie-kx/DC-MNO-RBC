...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate M0 Persistence on cross-parameter test split."
    )
...
def main():
    ...
    print("🚀 [Cross-Param M0 Persistence Evaluation]")
    ...
    with torch.no_grad():
        for batch_x_norm, batch_y_norm in test_loader:
            ...
            # M0 Persistence：直接使用当前最后一帧作为下一帧预测
            pred_y_norm = batch_x_norm[:, -4:, :, :]
    ...
    print("\n================ Cross-Param M0 Persistence Evaluation ================")
