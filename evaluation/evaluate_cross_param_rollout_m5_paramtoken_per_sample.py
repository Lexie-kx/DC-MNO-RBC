...
class RolloutDataset(Dataset):
        ...
        with h5py.File(DATA_PATH, "r") as f:
            for item in self.split_config:
                ...
                for local_traj_idx in range(num_traj):
                    original_traj_idx = int(traj_indices[local_traj_idx])
                    for t0 in range(max_start):
                        self.index.append(
                            {
                                "group": key,
                                "traj": local_traj_idx,
                                "original_traj": original_traj_idx,
                                "t0": t0,
                                "param": param,
                            }
                        )
    ...
    def __getitem__(self, idx):
        ...
        group_name = item["group"]
        traj_idx = item["traj"]
        original_traj_idx = item.get("original_traj", traj_idx)
        t0 = item["t0"]
        param = item["param"]
        ...
        return x0_phys, future, param, group_name, original_traj_idx, t0
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional path to save per-sample rollout errors. If omitted, use output path with _per_sample suffix."
    )
...
def main():
    ...
    if args.per_sample_output is None:
        if output_path.endswith(".csv"):
            per_sample_output_path = output_path[:-4] + "_per_sample.csv"
        else:
            per_sample_output_path = output_path + "_per_sample.csv"
    else:
        per_sample_output_path = resolve_path(project_root, args.per_sample_output)
    ...
    print(f"📌 Per-sample output: {per_sample_output_path}")
    ...
    with torch.no_grad():
        for batch_idx, (x0_phys, future_phys, param, group_names, original_traj_indices, t0_indices) in enumerate(loader):
            x0_phys = x0_phys.to(device)
            future_phys = future_phys.to(device)
            param = param.to(device)

            original_traj_indices = original_traj_indices.cpu().tolist()
            t0_indices = t0_indices.cpu().tolist()
            ...
            per_sample_pred_dict = {
                "M0": m0_preds,
                "M3-Delta": m3_preds,
                "M3-Delta-FiLM": film_preds,
                "M5-Delta-H4": m5_preds,
                "M5-Delta-FiLM-H4": m5_film_preds,
                "M5-Delta-ParameterToken-H4": m5_paramtoken_preds,
            }
            ...
            for horizon in horizons:
                ...
                # Per-sample Rel-L2 diagnostics.
                # 每一行对应一个样本、一个模型、一个 horizon。
                for ps_model_name, ps_preds in per_sample_pred_dict.items():
                    pred_h = ps_preds[idx]  # [B, 4, H, W]

                    err_sq = (pred_h - true_h) ** 2
                    tgt_sq = true_h ** 2

                    batch_n = true_h.shape[0]

                    for b in range(batch_n):
                        row = {
                            "sample_global_id": int(batch_idx) * int(args.batch_size) + int(b),
                            "group": group_names[b],
                            "trajectory": int(original_traj_indices[b]),
                            "t0": int(t0_indices[b]),
                            "model": ps_model_name,
                            "horizon": int(horizon),
                        }
                        ...
                        for c, field in enumerate(FIELD_ORDER):
                            field_sse = torch.sum(err_sq[b, c]).item()
                            field_tgt = torch.sum(tgt_sq[b, c]).item()
                            row[f"{field}_rel_l2_percent"] = (field_sse / (field_tgt + 1e-12)) ** 0.5 * 100.0

                        global_sse = torch.sum(err_sq[b]).item()
                        global_tgt = torch.sum(tgt_sq[b]).item()
                        row["global_rel_l2_percent"] = (global_sse / (global_tgt + 1e-12)) ** 0.5 * 100.0
    ...
    os.makedirs(os.path.dirname(per_sample_output_path), exist_ok=True)
    per_sample_df.to_csv(per_sample_output_path, index=False)

    print(f"\n✅ Rollout evaluation saved to: {output_path}")
    print(f"✅ Per-sample rollout diagnostics saved to: {per_sample_output_path}")
