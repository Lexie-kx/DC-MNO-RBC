...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Cross-parameter rollout evaluation for M0 / M3-Delta / FiLM / M5-H4 / ParamToken / M6 FieldWise-CouplingToken."
    )
    ...
    parser.add_argument(
        "--m6_fieldwise_couplingtoken_h4_checkpoint",
        type=str,
        required=True,
        help="Checkpoint for M6-FieldWise-CouplingToken-H4-unfreeze."
    )
    ...
    parser.add_argument(
        "--model_display_name",
        type=str,
        default="M6-FieldWise-CouplingToken-H4-unfreeze",
        help="Display name used in output tables for the M6 FieldWise CouplingToken model."
    )
...
def main():
    args = parse_args()
    M6_MODEL_NAME = args.model_display_name
    ...
    m6_fieldwise_couplingtoken_h4_ckpt = resolve_path(project_root, args.m6_fieldwise_couplingtoken_h4_checkpoint)
    ...
    print(f"📌 M6-FieldWise-CouplingToken-H4-unfreeze checkpoint: {m6_fieldwise_couplingtoken_h4_ckpt}")
    ...
    m6_fieldwise_couplingtoken_h4 = FieldWiseCouplingTokenFNO2d(
        in_channels=16,
        out_channels=4,
        modes1=16,
        modes2=16,
    ...
    m6_fieldwise_couplingtoken_h4 = load_model_state(
        m6_fieldwise_couplingtoken_h4,
        m6_fieldwise_couplingtoken_h4_ckpt,
        device,
    )
    m6_fieldwise_couplingtoken_h4.eval()
    ...
    with torch.no_grad():
        for batch_idx, (x0_phys, future_phys, param) in enumerate(loader):
            ...
            # M6-FieldWise-CouplingToken-H4-unfreeze
            # forward 同样是 model(x_norm, param)，但内部额外从 x_norm 提取 coupling token
            m6_fieldwise_couplingtoken_preds = rollout_m3_delta(
                model=m6_fieldwise_couplingtoken_h4,
                x0_phys=x0_phys,
                normalizer=normalizer,
                max_horizon=max_horizon,
            )
            ...
            for horizon in horizons:
                ...
                update_error_stats(
                    pred_phys=m6_fieldwise_couplingtoken_preds[idx],
                    true_phys=true_h,
                    stats_dict=stats_dict,
                    horizon=horizon,
                    model_name=M6_MODEL_NAME,
                )
    ...
    print_model_difference(
        wide_df=wide,
        model_a=M6_MODEL_NAME,
        model_b="M5-Delta-ParameterToken-H4",
        title=f"{M6_MODEL_NAME} - M5-Delta-ParameterToken-H4"
    )
    ...
    print_model_difference(
        ...
        model_a=M6_MODEL_NAME,
        model_b="M5-Delta-H4",
        title=f"{M6_MODEL_NAME} - M5-Delta-H4"
    )

    print_model_difference(
        wide_df=wide,
        model_a=M6_MODEL_NAME,
        model_b="M5-Delta-FiLM-H4",
        title=f"{M6_MODEL_NAME} - M5-Delta-FiLM-H4"
    )
