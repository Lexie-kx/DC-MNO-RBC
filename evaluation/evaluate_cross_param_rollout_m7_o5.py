...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Cross-parameter rollout evaluation including M7-ParamTokenOnly-O5-H4."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Checkpoint for M6-FieldWiseEncoder-H4-continue."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Checkpoint for M7-FieldCoupling-O5-H4."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Checkpoint for M7-ParamTokenOnly-O5-H4."
    )
...
def main():
    ...
    print(f"📌 M7-FieldCoupling-O5-H4 checkpoint: {m7_fieldcoupling_ckpt}")
    print(f"📌 M7-ParamTokenOnly-O5-H4 checkpoint: {m7_paramtoken_ckpt}")
    ...
    m7_fieldcoupling_h4 = load_model_state(
        m7_fieldcoupling_h4,
    ...
    m7_paramtoken_h4 = load_model_state(
        m7_paramtoken_h4,
        m7_paramtoken_ckpt,
        device,
    )
    ...
    with torch.no_grad():
        for batch_idx, (x0_phys, future_phys, param) in enumerate(loader):
            ...
            m6_fieldwise_preds = rollout_m3_delta(
            ...
            # Same autoregressive delta rollout; upgraded field-to-field coupling is inside the model.
            m7_fieldcoupling_preds = rollout_m3_delta(
            ...
            # Same autoregressive delta rollout, but model needs param input.
            m7_paramtoken_preds = rollout_film(
            ...
            for horizon in horizons:
                ...
                update_error_stats(
                    pred_phys=m6_fieldwise_preds[idx],
                    true_phys=true_h,
                    stats_dict=stats_dict,
                    horizon=horizon,
                    model_name="M6-FieldWiseEncoder-H4-continue",
                )

                update_error_stats(
                    pred_phys=m7_fieldcoupling_preds[idx],
                    true_phys=true_h,
                    stats_dict=stats_dict,
                    horizon=horizon,
                    model_name="M7-FieldCoupling-O5-H4",
                )

                update_error_stats(
                    pred_phys=m7_paramtoken_preds[idx],
                    true_phys=true_h,
                    stats_dict=stats_dict,
                    horizon=horizon,
                    model_name="M7-ParamTokenOnly-O5-H4",
                )
    ...
    # ============================================================
    # Compact print for current M7-ParamTokenOnly stage.
    ...
    focus = wide[wide["model"].isin(focus_models)].copy()
    focus["model"] = pd.Categorical(
        focus["model"],
        categories=focus_models,
        ordered=True,
    )
    focus = focus.sort_values(["model", "horizon"])

    print("\n================ Compact Rollout Summary: Current Stage ================")
    print("只打印当前 ParamToken-only 阶段最相关模型。完整结果已保存到 CSV。")
    print(focus[["model", "horizon", "buoyancy", "u_x", "u_y", "pressure", "global"]].to_string(index=False))

    global_table = focus.pivot_table(
        index="model",
        columns="horizon",
        values="global",
        observed=False,
    ).reindex(focus_models).reset_index()
    ...
    print_model_difference(
        wide_df=wide,
        model_a="M7-ParamTokenOnly-O5-H4",
        model_b="M6-FieldWiseEncoder-H4-continue",
        title="M7-ParamTokenOnly-O5-H4 - M6-FieldWiseEncoder-H4-continue"
    )

    print_model_difference(
        wide_df=wide,
        model_a="M7-ParamTokenOnly-O5-H4",
        model_b="M7-FieldCoupling-O5-H4",
        title="M7-ParamTokenOnly-O5-H4 - M7-FieldCoupling-O5-H4"
    )
    ...
    print_model_difference(
        ...
        title="M7-ParamTokenOnly-O5-H4 - M5-Delta-ParameterToken-H4"
    )
