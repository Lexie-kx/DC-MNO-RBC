...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate rollout physics diagnostics including M7-ParamTokenOnly-H4."
    )
    ...
    parser.add_argument("--m6_fieldwise_encoder_h4_checkpoint", type=str, required=True)
    parser.add_argument("--m7_fieldcoupling_checkpoint", type=str, required=True)
    parser.add_argument("--m7_paramtoken_checkpoint", type=str, required=True)
...
def main():
    ...
    m7_fieldcoupling_ckpt = resolve_path(project_root, args.m7_fieldcoupling_checkpoint)
    m7_paramtoken_ckpt = resolve_path(project_root, args.m7_paramtoken_checkpoint)
    ...
    print(f"📌 M6-FieldWiseEncoder-H4-continue checkpoint: {m6_fieldwise_encoder_h4_ckpt}")
    print(f"📌 M7-FieldCoupling-H4 checkpoint: {m7_fieldcoupling_ckpt}")
    print(f"📌 M7-ParamTokenOnly-H4 checkpoint: {m7_paramtoken_ckpt}")
    ...
    m5_delta_h4 = load_checkpoint(m5_delta_h4, m5_delta_h4_ckpt, device)
    ...
    m6_fieldwise_encoder_h4 = load_checkpoint(
        m6_fieldwise_encoder_h4,
        m6_fieldwise_encoder_h4_ckpt,
        device,
    )

    m7_fieldcoupling_h4 = M7FieldCouplingFNO2d(
        in_channels=CONTEXT_LENGTH * len(FIELD_ORDER),
        out_channels=len(FIELD_ORDER),
        modes1=16,
        modes2=16,
    ...
    m7_fieldcoupling_h4 = load_checkpoint(
        m7_fieldcoupling_h4,
        m7_fieldcoupling_ckpt,
        device,
    )

    m7_paramtoken_h4 = M7FieldWiseParamTokenFNO2d(
        in_channels=CONTEXT_LENGTH * len(FIELD_ORDER),
        out_channels=len(FIELD_ORDER),
        modes1=16,
        modes2=16,
    ...
    m7_paramtoken_h4 = load_checkpoint(
        m7_paramtoken_h4,
        m7_paramtoken_ckpt,
        device,
    )
    ...
    m7_fieldcoupling_h4.eval()
    m7_paramtoken_h4.eval()
    ...
    for batch_idx, batch in enumerate(loader, start=1):
        ...
        with torch.no_grad():
            ...
            seq_m5_delta_h4 = rollout_m3_delta(
                model=m5_delta_h4,
                x0_phys=x0_phys,
                normalizer=normalizer,
                max_horizon=max_horizon,
            ...
            seq_m5_delta_film_h4 = rollout_m3_delta_film(
            ...
            seq_m6_fieldwise_encoder_h4 = rollout_m3_delta(
            ...
            seq_m7_fieldcoupling_h4 = rollout_m3_delta(
                model=m7_fieldcoupling_h4,
                x0_phys=x0_phys,
                normalizer=normalizer,
                max_horizon=max_horizon,
            )

            seq_m7_paramtoken_h4 = rollout_m3_delta_film(
                model=m7_paramtoken_h4,
                x0_phys=x0_phys,
                param=param,
                normalizer=normalizer,
                max_horizon=max_horizon,
        ...
        pred_dict = {
            ...
            "M6-FieldWiseEncoder-H4-continue": seq_m6_fieldwise_encoder_h4,
            "M7-FieldCoupling-H4": seq_m7_fieldcoupling_h4,
            "M7-ParamTokenOnly-H4": seq_m7_paramtoken_h4,
        }
    ...
    # ============================================================
    # Compact print for current M7-ParamTokenOnly physics stage.
    # Full results are still saved in CSV above.
    # ============================================================
    ...
    row_map = {
        (row["model_name"], row["horizon"]): row
        for row in rows
    }

    print("\n================ Compact Physics Summary: Current Stage ================")
    print("只打印当前 ParamToken-only physics 阶段最相关模型。完整结果已保存到 CSV。")
    header = (
        f"{'model':<36} {'h':>4} "
    ...
    def print_physics_difference(model_a, model_b, title):
        """
        Print physics difference table: model_a - model_b.
        For these diagnostics, negative usually means model_a has lower error/magnitude.
        """
        print(f"\n================ {title}: Physics Difference ================")
        print("说明：前者 - 后者；负数通常表示前者指标更低。")
        header = (
            f"{'h':>4} "
            f"{'div_pred_mae':>14} {'div_err_mae':>14} "
            f"{'vort_rel_l2%':>14}"
        ...
        for h in horizons:
            ...
            diff = {
                "horizon": h,
                "div_pred_mae": a["div_pred_mae"] - b["div_pred_mae"],
                "div_error_mae": a["div_error_mae"] - b["div_error_mae"],
                "vorticity_rel_l2_percent": a["vorticity_rel_l2_percent"] - b["vorticity_rel_l2_percent"],
            }
            diff_rows.append(diff)

            print(
                f"{h:>4} "
                f"{diff['div_pred_mae']:>14.6e} "
                f"{diff['div_error_mae']:>14.6e} "
                f"{diff['vorticity_rel_l2_percent']:>14.6f}"
            )
    ...
    print_physics_difference(
        model_a="M7-ParamTokenOnly-H4",
        model_b="M6-FieldWiseEncoder-H4-continue",
        title="M7-ParamTokenOnly-H4 - M6-FieldWiseEncoder-H4-continue"
    )

    print_physics_difference(
        model_a="M7-ParamTokenOnly-H4",
        model_b="M7-FieldCoupling-H4",
        title="M7-ParamTokenOnly-H4 - M7-FieldCoupling-H4"
    )

    print_physics_difference(
        model_a="M7-ParamTokenOnly-H4",
        model_b="M5-Delta-ParameterToken-H4",
        title="M7-ParamTokenOnly-H4 - M5-Delta-ParameterToken-H4"
    )
