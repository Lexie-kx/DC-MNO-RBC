...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Seven-model M9-0 rollout physics diagnostics."
        )
    )
...
def main():
    ...
    m9_static_checkpoint = resolve_path(
        project_root,
        args.m9_static_checkpoint,
    )

    m9_state_checkpoint = resolve_path(
        project_root,
    ...
    print(
        "📌 M9-0a-Static: "
        f"{m9_static_checkpoint}"
    )
    print(
        "📌 M9-0b-Dynamic: "
        f"{m9_state_checkpoint}"
    )
    ...
    m8_full_static = load_checkpoint(
        m8_full_static,
        m8_full_static_checkpoint,
        device,
    )
    ...
    m8_param_conditioned = load_checkpoint(
        m8_param_conditioned,
        m8_param_conditioned_checkpoint,
        device,
    )

    m9_static = M9StaticAttentionFNO2d(
        in_channels=(
            CONTEXT_LENGTH
            * len(FIELD_ORDER)
        ),
        ...
        attention_hidden_channels=int(
            attention_config["hidden_channels"]
        ),
        attention_dropout=float(
            attention_config["coupling_dropout"]
        ),
    ...
    m9_static = load_checkpoint(
        m9_static,
        m9_static_checkpoint,
        device,
    )

    m9_state = M9StateAttentionFNO2d(
        in_channels=(
            CONTEXT_LENGTH
            * len(FIELD_ORDER)
        ),
        ...
        score_scale_mode=str(
            attention_config["score_scale_mode"]
        ),
        score_temperature=float(
            attention_config[
                "score_temperature"
    ...
    for batch_index, batch in enumerate(
        ...
        seq_m9_static = rollout_without_param(
            model=m9_static,
            x0_phys=x0_phys,
            normalizer=normalizer,
            max_horizon=max_horizon,
        ...
        seq_m9_state = rollout_without_param(
            model=m9_state,
            x0_phys=x0_phys,
            normalizer=normalizer,
            max_horizon=max_horizon,
    ...
    print("=" * 108)
    print(
        "说明：div_error 与 vort 是主要误差指标；"
        "div_pred 是预测散度幅值诊断。"
    )
    print(
        "完整 physics 指标与每组差值均已保存为 CSV。"
    )
    print("=" * 108)
