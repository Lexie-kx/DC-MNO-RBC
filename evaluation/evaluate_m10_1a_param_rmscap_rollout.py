...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "M10-1a focused Param-RMSCap rollout control: "
            "M6 vs original M10-Param vs M10-1a Param-RMSCap."
        )
    )
    ...
    parser.add_argument(
        "--original_param_checkpoint",
        required=True,
    )

    parser.add_argument(
        "--rmscap_param_checkpoint",
        required=True,
    )
...
def build_rmscap_param(
    ...
    if isinstance(payload, dict):
        ...
        if (
            ...
            raise RuntimeError(
                "M10-1a checkpoint is not Param mode."
            )
...
def main():
    ...
    original_param_path = (
        base.resolve_path(
            args.original_param_checkpoint
        )
    )

    rmscap_path = base.resolve_path(
        args.rmscap_param_checkpoint
    )
    ...
    with torch.no_grad():
        ...
        for (
            ...
            base.rollout_one_model(
                model_name="M10-Param",
                model=original_param,
                x0_phys=x0_phys,
                future_phys=future_phys,
                param=param,
                normalizer=normalizer,
