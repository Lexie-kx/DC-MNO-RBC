...
EXPECTED_M6_SHA = (
    "f7a91e4661703127d90c5a7e7ae686456f96ef3525e720dd605f422047001cab"
)

EXPECTED_M10_SHA = (
    "6854ab6bce91184c2df85e8a9e7e47934c1475c2b926a011db6dff857c165db3"
)
...
def main():
    ...
    #
    #   Legacy10        AUC = 0.876647
    #   TemporalAlpha20 AUC = 0.884566
    #
    ...
    # ====================================================
        ...
        # ========================================================
    # MULTI-SEED CLASSIFIER SANITY
    #
    # Same frozen classifier protocol as seed42.
    # AUC/accuracy are experimental outcomes for this seed,
    # so they are recorded rather than forced to reproduce
    # seed42 numerical values.
    # ========================================================
    ...
    for predictor_name in [
        ...
        row = predictor_sanity[
            predictor_sanity["predictor"] == predictor_name
        ]

        if len(row) != 1:
            raise RuntimeError(
                f"Missing classifier sanity row: {predictor_name}"
            )

        auc = float(
            row.iloc[0]["val_auc_on_bonly_states"]
        )

        acc = float(
            row.iloc[0]["val_accuracy_on_bonly_states"]
        )

        if not (0.0 <= auc <= 1.0):
            raise RuntimeError(
                f"Invalid AUC for {predictor_name}: {auc}"
            )

        if not (0.0 <= acc <= 1.0):
            raise RuntimeError(
                f"Invalid accuracy for {predictor_name}: {acc}"
            )

    print(
        "✅ Multi-seed classifier sanity PASS "
        "(AUC recorded, not seed42-locked)"
    )
