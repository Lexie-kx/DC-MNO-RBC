...
def main():
    ...
    # and therefore are recorded rather than required to equal
    # the seed42 values.
    # ========================================================
    ...
    for predictor_name in [
        ...
        row = predictor_sanity[
            predictor_sanity["predictor"]
            == predictor_name
        ]

        if len(row) != 1:
            raise RuntimeError(
                f"Missing classifier sanity row: "
                f"{predictor_name}"
            )
        ...
        if not (0.0 <= auc <= 1.0):
            raise RuntimeError(
                f"Invalid AUC for "
                f"{predictor_name}: {auc}"
            )

        if not (0.0 <= acc <= 1.0):
            raise RuntimeError(
                f"Invalid accuracy for "
                f"{predictor_name}: {acc}"
            )
