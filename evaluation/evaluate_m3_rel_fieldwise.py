...
def main():
    ...
    print("🚀 启动 M3-RelL2-Controlled 物理空间评估")
    ...
    print("📦 正在加载 M3-RelL2-Controlled 权重...")
    model_m3 = load_model(
    ...
    print("⏳ 开始 M3-RelL2 one-step physical-space evaluation...\n")
    ...
    with torch.no_grad():
        for batch_x_norm, batch_y_norm in val_loader:
            ...
            for k in ['buoyancy', 'u_x', 'u_y', 'pressure']:
                metrics['M3-RelL2-Controlled']['rel'][k] += batch_field_rel[k]
                metrics['M3-RelL2-Controlled']['mse'][k] += batch_field_mse[k]

            metrics['M3-RelL2-Controlled']['rel']['global'] += batch_total_rel
            metrics['M3-RelL2-Controlled']['mse']['global'] += batch_total_mse

    for metric_type in ['rel', 'mse']:
        for k in metrics['M3-RelL2-Controlled'][metric_type]:
            metrics['M3-RelL2-Controlled'][metric_type][k] /= num_batches

    fields = ['buoyancy', 'u_x', 'u_y', 'pressure']

    print("=" * 75)
    print(" 📊 Quantitative Evaluation: M3-RelL2-Controlled Physical Space")
    print("=" * 75)
    print(f"{'Field':<15} | {'Metric':<12} | {'M3-RelL2-Controlled':<22}")
    print("-" * 75)

    for field in fields:
        m3_rel = metrics['M3-RelL2-Controlled']['rel'][field] * 100
        m3_mse = metrics['M3-RelL2-Controlled']['mse'][field]
    ...
    m3_rel_g = metrics['M3-RelL2-Controlled']['rel']['global'] * 100
    m3_mse_g = metrics['M3-RelL2-Controlled']['mse']['global']
