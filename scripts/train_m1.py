...
def main():
    ...
    print(f"🚀 启动 [M1-MSE-Controlled] 训练 | 设备: {DEVICE}")

    wandb.init(
        project="DC-MNO",
        name="M1-MSE-Controlled-LR3e4-Cosine-Clip1",
        config={
    ...
    # 注意：MSE 不传 criterion，让 Trainer 默认使用 F.mse_loss
    trainer = Trainer(
        model=model,
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if epoch % 5 == 0:
            trainer.save_checkpoint(
                epoch,
                val_loss,
                filename=f"m1_mse_controlled_epoch_{epoch:02d}.pth"
            )
        ...
        if val_loss < best_val_loss:
            ...
            best_save_path = trainer.save_checkpoint(
                epoch,
                val_loss,
                filename="m1_mse_controlled_best.pth"
            )
