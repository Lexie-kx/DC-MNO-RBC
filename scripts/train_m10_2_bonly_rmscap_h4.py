...
def save_checkpoint(
    ...
    *,
    ...
    payload = {
        ...
        "physics_topology": {
            "path_a_output_injection": False,
            "buoyancy_anomaly_state_feature": True,
            "path_b": (
                "-u_dot_grad_b -> buoyancy normalized delta"
            ),
            "path_b_only_output_physics": True,
            "direct_ux_path": False,
            "direct_uy_path": False,
...
def main():
    ...
    if args.rollout_steps != 4:
        raise ValueError(
            "M10-2 B-only protocol is locked "
            "to rollout_steps=4."
        )
    ...
    experiment_labels = {
        "static": (
            "M10-2 Static-BOnly-RMSCap-H4"
        ),
        "param": (
            "M10-2 Param-BOnly-RMSCap-H4"
        ),
        "state": (
            "M10-2 State-BOnly-RMSCap-H4"
        ),
        "stateparam": (
            "M10-2 StateParam-BOnly-RMSCap-H4"
        ),
    ...
    print(
        "📌 Stage: M10-2 formal-candidate "
        "B-only fair re-training"
    )
    ...
    print(
        "📌 Path A residual injection: OFF"
    )

    print(
        "📌 b-mean_x(b) RMS retained only "
        "as compact state feature"
    )

    print(
        "📌 Only output physics path: "
        "-u·grad(b) -> buoyancy"
    )
