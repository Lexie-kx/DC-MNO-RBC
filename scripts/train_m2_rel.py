...
def main():
    ...
    print(f"🚀 启动 [M2-RelL2-Controlled] 训练 | 设备: {DEVICE}")
    print("👉 M2 = M1 + Ra/Pr 参数通道")
    print("👉 输入通道: 16 + 2 = 18")
    ...
    wandb.init(
        project="DC-MNO",
        name="M2-RelL2-Controlled-LR3e4-Cosine-Clip1",
        config={
            "experiment_type": "controlled_phase1_param_ablation",
            "architecture": "Parameter-conditioned Plain FNO",
            "normalization": "None",
            "parameter_conditioning": "log10(Ra), log10(Pr) as constant channels",
            "input_channels": 18,
    ...
    print(f"✅ M2 输入检查: X shape = {sample_x.shape}，应为 [B, 18, 256, 64]")
    print(f"✅ M2 目标检查: Y shape = {sample_y.shape}，应为 [B, 4, 256, 64]")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if epoch % 5 == 0:
            trainer.save_checkpoint(
                epoch,
                val_loss,
                filename=f"m2_rel_l2_controlled_epoch_{epoch:02d}.pth"
            )
        ...
        if val_loss < best_val_loss:
            ...
            best_save_path = trainer.save_checkpoint(
                epoch,
                val_loss,
                filename="m2_rel_l2_controlled_best.pth"
            )
