...
class MultiStepDeltaDataset(Dataset):
    """
    M5-Delta-FiLM-H4 dataset wrapper.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train M5-Delta-FiLM-H4: multi-step autoregressive Delta-FNO."
    )
    ...
    parser.add_argument(
        "--run_name",
        type=str,
        default="m5_delta_film_h4_unseen_pr",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional M3-Delta-FiLM checkpoint used to initialize M5 fine-tuning."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="Hidden dimension of FiLM parameter conditioner."
    )
...
def main():
    ...
    print(f"🚀 [M5-Delta-FiLM-H4] 启动训练 | 设备: {DEVICE}")
    print("👉 Base: M3-Delta-FiLM = Field-wise Normalized Delta-FNO + FiLM")
    ...
    wandb.init(
        ...
        name=RUN_NAME,
        config={
            "experiment_type": "m5_multistep_delta_film_prediction",
            "architecture": "FiLM-conditioned FNO",
            "base_model": "M3-Delta-FiLM",
    ...
    if INIT_CKPT is not None:
        ...
        print(f"🔁 正在从 M3-Delta-FiLM checkpoint 初始化模型: {INIT_CKPT}")
        ckpt = torch.load(INIT_CKPT, map_location=DEVICE)
        state_dict = ckpt["model_state_dict"] if isinstance(ckpt, dict) and "model_state_dict" in ckpt else ckpt
        model.load_state_dict(state_dict, strict=True)
        print("✅ M3-Delta-FiLM 权重加载成功，将进行 M5 multi-step fine-tuning")
