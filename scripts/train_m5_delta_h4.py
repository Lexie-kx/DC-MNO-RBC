...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train M5-Delta-H4: multi-step autoregressive Delta-FNO."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional M3-Delta checkpoint used to initialize M5 fine-tuning."
    )
...
def main():
    ...
    print(f"🚀 [M5-Delta-H4] 启动训练 | 设备: {DEVICE}")
    print("👉 Base: M3-Delta = Field-wise Normalized Delta-FNO")
    ...
    if INIT_CKPT is not None:
        ...
        print(f"🔁 正在从 M3-Delta checkpoint 初始化模型: {INIT_CKPT}")
        ckpt = torch.load(INIT_CKPT, map_location=DEVICE)
        state_dict = ckpt["model_state_dict"] if isinstance(ckpt, dict) and "model_state_dict" in ckpt else ckpt
        model.load_state_dict(state_dict, strict=True)
        print("✅ M3-Delta 权重加载成功，将进行 M5 multi-step fine-tuning")
