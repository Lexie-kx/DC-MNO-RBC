...
class MultiStepDeltaDataset(Dataset):
    """
    M5-Delta-ParameterToken-H4 dataset wrapper.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train M5-Delta-ParameterToken-H4: multi-step autoregressive Delta-FNO."
    )
    ...
    parser.add_argument(
        "--run_name",
        type=str,
        default="m5_delta_paramtoken_h4_unseen_pr",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional M5-Delta-H4 checkpoint used to initialize M5 fine-tuning."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="M5 rollout training uses more memory than one-step training. Start with 4."
    )
...
def main():
    ...
    if ROLLOUT_STEPS != 4:
        print(f"⚠️ 当前脚本名是 train_m5_delta_h4.py，但 rollout_steps={ROLLOUT_STEPS}")
    ...
    print(f"🚀 [M5-Delta-ParameterToken-H4] 启动训练 | 设备: {DEVICE}")
    print("👉 Base: M5-Delta-H4 = multi-step Delta-FNO + ParameterToken")
    print("👉 Upgrade: multi-step autoregressive rollout training")
    ...
    wandb.init(
        ...
        name=RUN_NAME,
        config={
            "experiment_type": "m5_multistep_delta_paramtoken_prediction",
            "architecture": "ParameterToken-conditioned FNO",
            "parameter_conditioning": "Additive ParameterToken per FNO block",
            "base_model": "M5-Delta-H4",
    ...
    if INIT_CKPT is not None:
        ...
        print(f"🔁 正在从 M5-Delta-H4 checkpoint 初始化 ParamToken 模型主干: {INIT_CKPT}")
        ckpt = torch.load(INIT_CKPT, map_location=DEVICE)
        state_dict = ckpt["model_state_dict"] if isinstance(ckpt, dict) and "model_state_dict" in ckpt else ckpt
        load_info = model.load_state_dict(state_dict, strict=False)
        print("✅ M5-Delta-H4 主干权重加载完成；新增 param_token.* 权重保持零初始化")
    ...
    print("\n🔥 开始 M5-Delta-ParameterToken-H4 多步自回归训练...")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        checkpoint_payload = {
            ...
            'experiment': 'M5-Delta-ParameterToken-H4',
            'parameter_conditioning': 'Additive ParameterToken per FNO block',
            'base_model': 'M3-Delta',
    ...
    print(f"\n✅ M5-Delta-ParameterToken-H4 训练完成!")
