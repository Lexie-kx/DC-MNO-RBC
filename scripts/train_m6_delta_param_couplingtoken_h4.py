...
from models.operators.fno2d_couplingtoken import ParamCouplingTokenFNO2d
from training.metrics import FieldWiseRelativeL2Loss


class MultiStepDeltaDataset(Dataset):
    """
    M6-Delta-ParamToken-CouplingToken-H4 dataset wrapper.
    ...
    def __getitem__(self, idx):
        ...
        if x_norm is None or y_seq_norm is None or param is None:
            shapes = [
                tuple(obj.shape) if torch.is_tensor(obj) else type(obj).__name__
                for obj in item
            ]
            raise RuntimeError(
                "❌ 无法解析 RBCDataset 返回值中的 x_norm / y_seq_norm / param。"
                f" item shapes/types = {shapes}。"
                " 请确认 return_sequence=True 且 return_params=True。"
            )
        ...
        context_norm = x_norm.reshape(self.context_length, 4, h, w)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train M6-Delta-ParamToken-CouplingToken-H4: multi-step autoregressive Delta-FNO."
    )
    ...
    parser.add_argument(
        "--run_name",
        type=str,
        default="m6_delta_param_couplingtoken_h4_unseen_pr",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional M5-Delta-ParameterToken-H4 checkpoint used to initialize M6 CouplingToken fine-tuning."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="M6 rollout training uses more memory than one-step training. Start with 4."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="Hidden dimension of ParameterToken conditioner."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="Hidden dimension of CouplingToken conditioner."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Print batch-level progress every N batches. Use 0 to disable."
    )
...
def autoregressive_multistep_loss(
    ...
    param = param.to(device=context_norm.device, dtype=context_norm.dtype)
...
def main():
    ...
    if ROLLOUT_STEPS != 4:
        print(f"⚠️ 当前脚本名是 train_m6_delta_param_couplingtoken_h4.py，但 rollout_steps={ROLLOUT_STEPS}")
    ...
    print(f"🚀 [M6-Delta-ParamToken-CouplingToken-H4] 启动训练 | 设备: {DEVICE}")
    print("👉 Base: M5-Delta-ParameterToken-H4 + CouplingToken")
    print("👉 Upgrade: multi-step autoregressive rollout training with coupling token")
    ...
    wandb.init(
        ...
        name=RUN_NAME,
        config={
            "experiment_type": "m6_multistep_delta_param_couplingtoken_prediction",
            "architecture": "ParamToken + CouplingToken conditioned FNO",
            "parameter_conditioning": "Additive ParameterToken + CouplingToken per FNO block",
            "coupling_conditioning": "Field statistics mean/std/rms -> additive coupling token per FNO block",
            "base_model": "M5-Delta-ParameterToken-H4",
            "upgrade": "CouplingToken fine-tuning on M5-Delta-ParameterToken-H4",
    ...
    if INIT_CKPT is not None:
        ...
        print(f"🔁 正在从 M5-Delta-ParameterToken-H4 checkpoint 初始化 M6 模型: {INIT_CKPT}")
        ckpt = torch.load(INIT_CKPT, map_location=DEVICE)
        state_dict = ckpt["model_state_dict"] if isinstance(ckpt, dict) and "model_state_dict" in ckpt else ckpt
        load_info = model.load_state_dict(state_dict, strict=False)
        print("✅ M5-Delta-ParameterToken-H4 权重加载完成；新增 coupling_token.* 权重保持零初始化")
    ...
    print("\n🔥 开始 M6-Delta-ParamToken-CouplingToken-H4 多步自回归训练...")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        checkpoint_payload = {
            ...
            'experiment': 'M6-Delta-ParamToken-CouplingToken-H4',
            'parameter_conditioning': 'Additive ParameterToken + CouplingToken per FNO block',
            'coupling_conditioning': 'Field statistics mean/std/rms -> additive coupling token per FNO block',
            'base_model': 'M5-Delta-ParameterToken-H4',
    ...
    print(f"\n✅ M6-Delta-ParamToken-CouplingToken-H4 训练完成!")
