...
from models.operators.fno2d_fieldwise_couplingtoken import FieldWiseCouplingTokenFNO2d
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train M6-FieldWise-CouplingToken-H4: field-wise encoder plus coupling token, no parameter token."
    )
    ...
    parser.add_argument(
        "--run_name",
        type=str,
        default="m6_fieldwise_couplingtoken_h4_unseen_pr",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional M6-FieldWiseEncoder-H4 checkpoint used to initialize shared weights before training CouplingToken."
    )
    ...
    parser.add_argument(
        "--freeze_backbone",
        action="store_true",
        help="Freeze FieldWiseEncoder/FNO backbone and train only coupling_token. Recommended for clean CouplingToken-only ablation."
    )
...
def main():
    ...
    if ROLLOUT_STEPS != 4:
        print(f"⚠️ 当前脚本名是 train_m6_fieldwise_couplingtoken_h4.py，但 rollout_steps={ROLLOUT_STEPS}")
    ...
    print(f"🚀 [M6-FieldWise-CouplingToken-H4] 启动训练 | 设备: {DEVICE}")
    print("👉 Base: M6-FieldWiseEncoder-H4 warm start + zero-init CouplingToken")
    print("👉 Upgrade: field-wise encoder + coupling token + multi-step autoregressive rollout training")
    ...
    wandb.init(
        ...
        name=RUN_NAME,
        config={
            "experiment_type": "m6_fieldwise_couplingtoken_h4",
            "architecture": "FieldWiseEncoder + CouplingToken",
            "base_model": "M6-FieldWiseEncoder-H4",
            "upgrade": "field-wise encoder + zero-init coupling token + multi-step autoregressive training",
    ...
    if INIT_CKPT is not None:
        ...
        print(f"🔁 正在从 M6-FieldWiseEncoder-H4 checkpoint 初始化 M6-FieldWise-CouplingToken-H4 共享权重: {INIT_CKPT}")
        ...
        print("   说明：coupling_token 是新模块；其最后一层零初始化，初始接近 FieldWiseEncoder baseline。")
        print("✅ M6-FieldWiseEncoder-H4 共享权重加载成功，将训练 M6-FieldWise-CouplingToken-H4")

    if args.freeze_backbone:
        print("🧊 freeze_backbone=True: 冻结 FieldWiseEncoder/FNO 主干，只训练 coupling_token。")
        for name, param in model.named_parameters():
            param.requires_grad = name.startswith("coupling_token.")
    ...
    print("\n🔥 开始 M6-FieldWise-CouplingToken-H4 多步自回归训练...")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        checkpoint_payload = {
            ...
            'experiment': 'M6-FieldWise-CouplingToken-H4',
            'base_model': 'M6-FieldWiseEncoder-H4',
